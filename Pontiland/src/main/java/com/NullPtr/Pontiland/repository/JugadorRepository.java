...
/**
 * Repositorio encargado de gestionar las operaciones relacionadas con la entidad Jugador
 * en la base de datos, como la inserción de nuevos jugadores.
 */
public class JugadorRepository implements IJugadorRepository {
     ...
     */
    JugadorRepository(IDataService dataService) {this.dataService=dataService;}
    ...
    public void newPlayer(Jugador newPlayer, long partidaID, int icono){
        ...
        String nuevoJugador = "INSERT INTO Jugador(NumJugador, NombreJugador, IconoID, Partida) VALUES(?, ? , ? , ? )";
        try {
    ...
    public Jugador obtenerJugador(int id){
        Connection conn = dataService.createConnection();
        String jugador = "SELECT * FROM JUGADOR LEFT JOIN ADQUISICIONES ON JUGADOR.JUGADORID=ADQUISICIONES.JUGADORID WHERE JUGADOR.JUGADORID= ?";
        try {
            PreparedStatement consultarJugador = conn.prepareStatement(jugador);
            consultarJugador.setInt(1, id);
            ResultSet rsPartida = consultarJugador.executeQuery();
            Jugador jugador1 = new Jugador(rsPartida.getInt("Jugador.Dinero"), rsPartida.getString("Jugador.Nombre"), )
        } catch (SQLException e) {
            throw new RuntimeException(e);
        }
    }
