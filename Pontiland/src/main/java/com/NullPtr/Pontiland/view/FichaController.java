...
public class FichaController {
  ...
  public FichaController(AssetManager assetManager, BulletAppState bullet, Node rootNode, Vector3f boardFirstPosition, int totalCasillas) {
  ...
  public void loadFichasModels(Ficha[] data) {
    for (int i = 0; i < data.length; i++) {
      ...
      Vector3f placed;
      if (i < 2) {
        placed = new Vector3f(boardFirstPosition.getX() + i * 0.5f, 0.5f, boardFirstPosition.getZ());
      } else {
        placed = new Vector3f(boardFirstPosition.getX() + i % 2 * 0.5f, 0.5f, boardFirstPosition.getZ() + 0.5f);
      }
