...
class JugadorTest {
  ...
  void settersAndGettersTest() {
    ...
    assertEquals(
        "NuevoNombre", jugador.getNombreJugador(), "El nombre del jugador debe actualizarse");
  ...
  void setJugadorIdBelowZeroTest() {
    ...
    IllegalArgumentException exception =
        assertThrows(
            IllegalArgumentException.class,
            () -> jugador.setJugadorId((byte) -1),
            "Debe lanzar excepción si el jugadorId es menor que 0");
    assertEquals(
        "Identificador del jugador es inválido",
        exception.getMessage(),
        "El mensaje de excepción debe coincidir");
  }
  ...
  void setJugadorIdValidTest() {
    ...
    for (byte id = 0; id <= 4; id++) {
      ...
      assertEquals(
          id, jugador.getJugadorId(), "El jugadorId debe establecerse correctamente a " + id);
    }
  }
  ...
  void setNombreJugadorValidTest() {
    ...
    jugador.setNombreJugador("NuevoNombre");
    assertEquals(
        "NuevoNombre", jugador.getNombreJugador(), "El nombre debe actualizarse correctamente");
  }
  ...
  void invalidArgumentsTest() {
    ...
    assertThrows(
        IllegalArgumentException.class,
        () -> new Jugador(1, 2, "Nombre", 1, 10, false, -1, 100L,0),
    ...
    assertThrows(
        ...
        "Posición no puede ser menor que 1");
    assertThrows(
        IllegalArgumentException.class,
        () -> jugador.setPosicion(41),
        "Posición no puede ser mayor que 40");
    assertThrows(
        IllegalArgumentException.class,
        () -> jugador.setPropiedades(null),
        "Lista de propiedades no puede ser null");
    assertThrows(
        IllegalArgumentException.class,
        () -> jugador.setJugadorId((byte) -1),
        "JugadorId no puede ser menor que 0");
    assertThrows(
        IllegalArgumentException.class,
        () -> jugador.setJugadorId((byte) 5),
  ...
  /**
   * Verifica que el constructor por defecto lanza excepción si jugadorId es menor que 1 o mayor que
   * 4
   */
  @Test
